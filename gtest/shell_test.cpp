...
protected:
    virtual void SetUp() {
        ...
        if (pid == 0) {
            ...
            // execlp("valgrind", "valgrind", "--leak-check=yes", "./shell_sol", NULL);
        }
    }
...
int writeInput(const char* command, bool closePipe) {
    ...
    if (select_status == -1) {
        ...
        fprintf(stderr, "Timeout waiting for input_fd to become ready for writing\n");
        return -1;
    }
...
TEST_F(ShellTest, TestForkInBackground) {
    ...
    while (fgets(buffer, sizeof(buffer), ps) != NULL) {
        ...
        if (std::regex_search(child_pid, num, regex)) {
            if(children.count(std::stoi(num.str()))) break;
...
TEST_F(ShellTest, Exit) {
    ...
    if (ret == 0) EXPECT_TRUE(false) << "Shell did not exit within the timeout period";
...
TEST_F(ShellTest, ExitWithArgument) {
    ...
    if (ret != 0) EXPECT_TRUE(false) << "Shell exited with an invalid argument period";
...
TEST_F(ShellTest, CdBackward) {
    ...
    if (getcwd(cwd, sizeof(cwd)) != NULL) {
        std::string expected_output = current_dir.substr(0, current_dir.find_last_of("/\\"));
...
TEST_F(ShellTest, InternalHelpCommand) {
    ...
    std::string expected_output = "'cd' is a builtin command";
    ...
    bool found = true;
    if (output.find(expected_output) == std::string::npos) found = false;
...
TEST_F(ShellTest, ExternalHelpCommand) {
    ...
    std::string expected_output = "'echo' is an external command or application";
...
TEST_F(ShellTest, IllegalHistoryCommand) {
    ...
    //make sure the first line isn't the command prompt. Should be an error message instead
    bool errorCommand = (line.find(getcwd(NULL, 0)) == std::string::npos) && line.size() != 0;
    ...
    std::string out_line;
    int num_of_lines = 0;
    while(std::getline(is, out_line)) num_of_lines++;
...
TEST_F(ShellTest, CtrlCInformation) {
    ...
    // Check that the shell has resumed normal operation and is showing the command prompt
    char cwd[1024];
    if (getcwd(cwd, sizeof(cwd)) != NULL) {
        std::string current_dir = cwd;
...
TEST_F(ShellTest, CtrlCNoCrash){
    ...
    if (ret != 0) EXPECT_TRUE(false) << "Shell exited";
...
TEST_F(ShellTest, CDWithoutArgument) {
    ...
    if(internalScore+externalScore+historyScore < 80) GTEST_SKIP();
...
TEST_F(ShellTest, ChangeToHomeDirectory) {
    ...
    int create = mkdir(directory_path.c_str() , S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
    ASSERT_TRUE(create == 0);
    ...
    adjustScore(output.find(expected_output) != std::string::npos, 4, extraCredit);
...
TEST_F(ShellTest, cdBack) {
    ...
    adjustScore(output.find(parsedOutput(prev_path)) != std::string::npos, 5, extraCredit);
